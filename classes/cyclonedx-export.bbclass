...
def export_cyclonedx(d):
    ...
    # Clear only the *contents* of the tmp export dir, never the directory itself.
    # Deleting and re-creating the directory releases its inode. Under pseudo/fakeroot,
    # a subsequent task (e.g. do_image_cpio creating cpio_append) may reuse that inode,
    # causing a "path mismatch" abort in pseudo's database.
    import shutil
    if os.path.exists(tmp_export_dir):
        for _item in os.listdir(tmp_export_dir):
            _item_path = os.path.join(tmp_export_dir, _item)
            if os.path.islink(_item_path) or os.path.isfile(_item_path):
...
def get_cyclonedx_deploy_path(d, path_variable_name):
    import os
...
def cleanup_cyclonedx_deploy_targets(d):
    import os

    for var in (
        "CYCLONEDX_EXPORT_SBOM",
        "CYCLONEDX_EXPORT_VEX",
        "CYCLONEDX_EXPORT_SBOM_LINK",
        "CYCLONEDX_EXPORT_VEX_LINK",
    ):
        path = get_cyclonedx_deploy_path(d, var)
        if not path or not os.path.lexists(path):
            continue
        os.unlink(path)
        bb.note(f"CycloneDX: removed stale deploy target {path}")

def deploy_cyclonedx_from_tmp(d):
    import os
    import shutil
    ...
    if not export_dir or not tmp_export_dir or not os.path.exists(tmp_export_dir):
        bb.note("CycloneDX: nothing to deploy")
        return

    bb.utils.mkdirhier(export_dir)

    for root, _, files in os.walk(tmp_export_dir):
        rel_dir = os.path.relpath(root, tmp_export_dir)
        dst_dir = export_dir if rel_dir == "." else os.path.join(export_dir, rel_dir)
        ...
        for name in files:
            ...
            bb.note(f"CycloneDX: deployed {os.path.relpath(src_path, tmp_export_dir)} to {export_dir}")
        ...
        for name in os.listdir(root):
            ...
            bb.note(f"CycloneDX: deployed symlink {os.path.relpath(src_path, tmp_export_dir)} to {export_dir}")

python do_export_cyclonedx() {
    # 1. Generate the files while the package manager (DNF) is intact
    export_cyclonedx(d)
    
    # 2. Deploy immediately (Image recipes only)
    if d.getVar('IMAGE_ROOTFS'):
        cleanup_cyclonedx_deploy_targets(d)
        deploy_cyclonedx_from_tmp(d)
}

# PREPEND (=+) to postprocess. 
# This ensures it runs after installation, but BEFORE Yocto uninstalls DNF.
ROOTFS_POSTPROCESS_COMMAND =+ "do_export_cyclonedx; "

# ====================================================================
# NON-IMAGE RECIPE FALLBACK
# ====================================================================
do_deploy_cyclonedx[nostamp] = "1"
do_deploy_cyclonedx[vardeps] += "CYCLONEDX_EXPORT_DIR CYCLONEDX_EXPORT_SBOM CYCLONEDX_EXPORT_VEX CYCLONEDX_EXPORT_SBOM_LINK CYCLONEDX_EXPORT_VEX_LINK CYCLONEDX_SPEC_VERSION"

python do_deploy_cyclonedx() {
    # RUNTIME CHECK: The Ultimate Guardrail.
    # If this recipe has a rootfs, it's an image. Exit immediately and safely.
    if d.getVar('IMAGE_ROOTFS'):
        bb.note("CycloneDX: Standalone deploy skipped. Image deployment is handled by ROOTFS hook.")
        return
...
python () {
    # Try to prevent the task from even being added to images at parse time
    if not d.getVar('IMAGE_ROOTFS'):
        bb.build.addtask("do_deploy_cyclonedx", "do_build", "do_install", d)
    else:
        # If it IS an image, ensure the rogue task is explicitly removed
        bb.build.deltask("do_deploy_cyclonedx", d)
}